...
3. Найти заказы, сделанные клиентом по имени "Mia".
db.orders.aggregate([
    {$match: {customerFirstName: {$eq: "Mia"}}}
])
4. Найти первые 5 заказов, отсортированные по дате заказа по убыванию.
db.orders.aggregate([
    {$sort: {orderDate: -1}},
    {$limit: 5}
])
5. Найти заказы, где сумма заказа находится в диапазоне от 100 до 500, отсортировать их по возрастанию суммы и пропустить первую запись.
db.orders.aggregate([
    {$match: {totalAmount: {$gte:100,$lte:500}}}, 
    {$sort: {totalAmount: 1}},
    {$skip: 1}
])
6. Изменить статус заказа с orderId 2 на "shipped".
db.orders.updateOne({orderId: {$eq: 2}}, {$set: {status: "shipped"}})
7. Найдите общее количество заказов.
db.orders.count()
8. Используя агрегацию, найдите среднee значение общей суммы заказов(totalAmount).
db.orders.aggregate([
    {
        $group: {
            _id: null,  // Используйте null для группировки всех документов вместе
            averageTotalAmount: { $avg: '$totalAmount' }
        }
    }
])

9.Используя агрегацию, вывести суммы общей суммы заказов(totalAmount) по статусам доставки(status).
db.orders.aggregate([
    {
        $group: {
            _id: '$status',  // Группируем по полю status
            totalAmount: { $sum: '$totalAmount' }  // Суммируем totalAmount для каждой группы
        }
    }
])